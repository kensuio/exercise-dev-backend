...
final case class RefOneForgeCache(config: OneForgeCacheConfig, clock: Clock, ref: Ref[Option[CacheContent]]) extends OneForgeCache {
  def getOrUpdate(pair: Rate.Pair, fetchUpdated: IO[OneForgeError, List[Rate]]): IO[OneForgeError, Rate] =
    ref.get.flatMap { cacheRef =>
      ...
      ZIO.fromOption(rates.find(_.pair == pair)).orElseFail(OneForgeError.MissingPair(pair))
    )
}
